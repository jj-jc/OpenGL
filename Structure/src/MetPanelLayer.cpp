...
{
	vertices[0][0] = m_pos[0];				vertices[0][1] = m_pos[0] - m_height;	vertices[0][2] = m_pos[0]; // 0
	vertices[1][0] = m_pos[0] + m_width;	vertices[1][1] = m_pos[0] - m_height;	vertices[1][2] = m_pos[0]; // 1
	vertices[2][0] = m_pos[0] + m_width;	vertices[2][1] = m_pos[0]; 				vertices[2][2] = m_pos[0]; // 2
	vertices[3][0] = m_pos[0]; 				vertices[3][1] = m_pos[0]; 				vertices[3][2] = m_pos[0]; // 3
}


MetPanelLayer::MetPanelLayer()
	:Layer("MetPanel"), m_BackgroundSquare({-0.5f, 0.5f, 0.0f}, 1.0f, 1.0f), 
	m_ExternalEdgeSquare({-0.55f, 0.55f, -0.1f}, 1.10f, 1.1f),
	m_InternalEdgeSquare({-0.20f, 0.55f,  0.1f}, 0.06f, 1.1f)
{

}

MetPanelLayer::MetPanelLayer(const std::string& name)
	:Layer(name), m_BackgroundSquare({-0.7f, 0.5f, 0.0f}, 1.0f, 1.0f), 
	m_ExternalEdgeSquare({-0.55f, 0.55f, -0.1f}, 1.10f, 1.1f),
	m_InternalEdgeSquare({-0.20f, 0.55f,  0.1f}, 0.06f, 1.1f)
{
...
void MetPanelLayer::update(double ts)
{
	...
	model = glm::translate(translationMat, m_SquareTranslation);
...
void MetPanelLayer::renderImGui()
{
	...
	if(ImGui::SliderFloat3("Position", glm::value_ptr(m_SquareTranslation), -2.0f, 2.0f))
		m_SquareTranslation = m_SquareTranslation;
