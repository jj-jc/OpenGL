...
int main(void)
{
	std::cout << "Translation example:" << std::endl;
	// ---------------------------------------------------------------------
    ...
    window = glfwCreateWindow(SCR_WIDTH, SCR_HEIGHT, "Translation example", NULL, NULL);
    if (!window)
    {
...
//	coutMatrix(&trans);
    ...
    while (!glfwWindowShouldClose(window))
    {
    	...
    	glm::mat4 trans = glm::mat4(1.0f);
    	// It is important to understand that the order is the other way around.
    	//to rotate and then translate, first add the translate matrix and then the rotate matrix ( as the multiplication)
